...
def parse_args():
    ...
    parser.add_argument("--keep-dc", action="store_true", help="Do not suppress the central DC local oscillator (LO) leakage spike in PSD peak finding.")
...
def run_sdr_command(cmd, timeout=None):
    """Run an SDR command and trap common cryptic errors."""
    try:
        if timeout:
        ...
        print(f"\n❌ SDR Command Failed: {' '.join(cmd)}", file=sys.stderr)
        
        if "usb_claim_interface" in stderr or "device or resource busy" in stderr or "claim failed" in stderr:
            print("   ↳ Error: SDR is busy. Do you have SDRconnect, SDRuno, CubicSDR, or GQRX open in the background?", file=sys.stderr)
        elif "no supported devices found" in stderr or "no device" in stderr:
            print("   ↳ Error: SDR not detected. Is it plugged in and passed through to your VM?", file=sys.stderr)
        elif "cb transfer status: 1" in stderr or "dropped" in stderr:
            print("   ↳ Error: USB transfer dropped. Try a different USB port or a shorter cable.", file=sys.stderr)
        else:
            print(f"   ↳ Raw Error: {e.stderr.strip() if e.stderr else 'Unknown error'}", file=sys.stderr)
        sys.exit(1)
